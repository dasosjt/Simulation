...
population = np.random.uniform(0,30,(population_size,population_gen))
...
while N < 100:
    ...
    for n in range(len(fitness)):
        if sum(fitness[n])/3 < 0.8 or fitness[n][0] == 0 or fitness[n][1] == 0 or fitness[n][2] == 0.5:
            ...
            if new_fen == 1:
                gen0 = np.random.uniform(0,30,1)
                gen1 = np.random.uniform(0,30,1)
                print ("Fen number {} with [{}, {}].. and new Fen".format(n, gen0, gen1))
                population[n][0] = gen0
                population[n][1] = gen1
            else:
                if mutation == 1:
                    mut0 = np.random.uniform(-30,30,1)
                    mut1 = np.random.uniform(-30,30,1)
